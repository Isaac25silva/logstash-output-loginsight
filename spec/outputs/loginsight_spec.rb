...
class LogStash::Outputs::Loginsight
  attr_writer :agent
...
describe LogStash::Outputs::Loginsight do
  ...
  describe "when num requests > token count" do
    ...
    subject {
      LogStash::Outputs::Loginsight.new("url" => url,
                                        "http_method" => method,
                                        "pool_max" => pool_max)
    }
  ...
  shared_examples("verb behavior") do |method, async_type|
    subject { LogStash::Outputs::Loginsight.new("url" => url, "http_method" => method, "pool_max" => 1) }
  ...
  LogStash::Outputs::Loginsight::VALID_METHODS.each do |method|
  ...
  describe "integration tests" do
    ...
    subject { LogStash::Outputs::Loginsight.new(config) }
